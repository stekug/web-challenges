...
function findById(employees, id) {
  const foundID = employees.find((employeeID) => employeeID.id === id);
  return foundID;
}
...
function findByCitySubString(employees, string) {
  const foundCity = employees.find((employeeCity) =>
    employeeCity.city.includes(string)
  );
  return foundCity;
}
...
function findByProfessionSubStringAndAge(employees, string, age) {
  ...
  const foundEmployee = ageArray.find((employee) =>
